...
function append(){
    ...
    electronicData.forEach(function(ele,i){
      ...
      btn.addEventListener("click",()=>{
          ...
          let newObj=new CFun(image.src,name.innerText,price.innerText)
          newArray.push(newObj);
          document.getElementById("cart_count").innerText=count;
          localStorage.setItem("cart_items",JSON.stringify(newArray))
            localStorage.setItem("items_count",(count))
            // people who is working on cart page can get the data from local storage using above mention keys +there are two keys one 
            // for getting the data to  append on cart page and other key is for  updating the count of items. of products added 
            // to the cart on cart icon.
